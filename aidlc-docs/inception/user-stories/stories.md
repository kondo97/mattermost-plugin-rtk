...
| Join button in post | Yes — disabled when already in that call | Yes — "Leave" when in call, "Join" when not | Aligned |
...
**Acceptance Criteria:**
- [ ] A toast bar appears above the message input area for channel members who are not in the call when a call is active.
- [ ] The toast bar shows the call start time and participant avatars.
- [ ] The toast bar has a dismiss button; dismissing it hides it for the local user only.
...
### US-010: Custom Post Shows "Leave" Button When Already in Call
**Persona**: Channel Member (Web)
**As a** channel member who is already in the call, **I want** the post card to show a "Leave" button instead of "Join call", **so that** I am not confused about my call state.

**Acceptance Criteria:**
- [ ] For users currently in the call, the post card shows a "Leave" button.
- [ ] The "Leave" button is enabled and triggers the leave flow (see US-016).
- [ ] The "Join call" button is not shown to users already in the call.
...
### US-014: Leave a Call via the Post Card "Leave" Button
**Persona**: Channel Member (Web)
**As a** channel member in a call, **I want to** click "Leave" in the call post card, **so that** I can exit the call from the Mattermost interface.

**Acceptance Criteria:**
- [ ] Clicking "Leave" sends a leave request to the server.
- [ ] The user is removed from the `participants` list in KVStore.
- [ ] A `custom_cf_user_left` WebSocket event is emitted to all connected clients.
- [ ] The call page tab is not automatically closed (the user may close it manually).
...
**As a** call host, **I want to** end the call for all participants, **so that** I can close the session without waiting for everyone to leave individually.

**Acceptance Criteria:**
- [ ] The host has an "End call" action in the call UI.
...
### US-017: Register VoIP Device Token
**Persona**: Mobile User
**As a** mobile user, **I want** the app to register my VoIP push token with the plugin, **so that** I receive native incoming call notifications.

**Acceptance Criteria:**
- [ ] The mobile app calls `POST /api/v1/voip/token` with the device token.
- [ ] iOS VoIP tokens are stored with the prefix `apple_voip:{token}` in KVStore, keyed by user ID.
- [ ] Token registration requires a valid `Mattermost-User-ID` header.
...
**Acceptance Criteria:**
- [ ] When a call starts, the plugin sends a push notification to all channel members who are not the caller, via the Mattermost push notification infrastructure.
- [ ] The notification payload includes: `channel_id`, `sender_id`, `sender_name`, `channel_name`, `ack_id`, `uuid`, `sub_type: "calls"`, `root_id`, `server_id`.
- [ ] The notification is delivered using the registered VoIP token.
...
### US-026: Host Ends Call for All — Explicit End Action
**Persona**: Channel Member (Web) — Host
**As a** call host, **I want to** end the call for everyone even if other participants are still present, **so that** I can close the session intentionally without waiting for everyone to leave.

**Acceptance Criteria:**
- [ ] The call creator has an "End call" action available (in addition to the standard "Leave" action).
- [ ] Triggering "End call" immediately ends the call for all participants regardless of how many are present.
- [ ] `end_at` is set in KVStore; `custom_cf_call_ended` is emitted; post switches to ended state; toast bar is dismissed for all.
